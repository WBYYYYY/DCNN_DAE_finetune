...
train_path_nameH=r'D:\UNET\data\train_H/'
train_path_nameT=r'D:\UNET\data\train_T/'

test_path_nameH=r'D:\UNET\data\test_H/'
test_path_nameT=r'D:\UNET\data\test_T/'
...
def reader(datadir_H,datadir_T,batchsize):
    ...
    while 1: 
        ...
        for i in range(idx,idx+batchsize): 
            ...
            img_phase = imgprocess(phase_filepath)
...
from tensorflow.keras.layers import Input, Activation, Add,MaxPooling2D,UpSampling2D,LeakyReLU,Conv2DTranspose,Conv2D,BatchNormalization,Dense
...
#model = load_model(r'D:\SMC\U-net\noise\model\u_FAnoise2deform.h5', custom_objects={'myssimcost': myssimcost})
...
tensorboard = TensorBoard(log_dir='D:\UNET\data/logs', write_images=True, update_freq=1)
model_checkpoint = ModelCheckpoint(r'D:\UNET\data\u_128.h5', monitor='loss', verbose=1, save_best_only=True)
...
hist = model.fit(train_reader,steps_per_epoch=np.ceil(train_len/batch_size),epochs=100,validation_data=test_reader,validation_steps=np.ceil(test_len/batch_size),callbacks=[model_checkpoint, tensorboard])
